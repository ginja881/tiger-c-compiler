...
struct A_Field_ {
     ...
     union {
	  ...
	  struct {struct A_Exp_* left_id; struct A_Exp_* right_id;} ref_field;
	  struct {string id; struct A_Exp_* value;} item_field;
	  struct {string type_id; struct A_FieldList_* record_def_fields;} record_field;
	  struct {struct A_FieldList_* record_type_fields;} type_record_field;
     } u;
};
